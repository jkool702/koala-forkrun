...
export FORKRUN_EXTRA_VARS="PATH TEMP_DIR"
...
check_ports() {
    ...
    if [ -z "$LISTENING_PORTS" ]; then
        check_security "Port Scanning" "WARN" "Port scanning failed due to missing tools. Ensure 'ss' or 'netstat' is installed." "024.txt"
    else
